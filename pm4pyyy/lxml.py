...
def parse_element(bpmn_graph, counts, curr_el, parents, incoming_dict, outgoing_dict, flows_name, nodes_dict, nodes_bounds,
    ...
    if tag.endswith("subprocess"): # subprocess invocation
        ...
        for child in curr_el:
            if child.tag.lower().endswith("datainputassociation"):
                if node_str not in data_input_associations:
                    data_input_associations[node_str] = []
    ...
    #         outgoing_dict[curr_el.text.strip()] = node
        ...
        seq_flow_name = curr_el.get("name")
        ...
        flows_name[seq_flow_id] = seq_flow_name
    ...
    #    outgoing_dict[seq_flow_id] = source_ref
        ...
        name = curr_el.get("name").replace("\r", "").replace("\n", "") if "name" in curr_el.attrib else ""
        for child in curr_el:
            if child.tag.lower().endswith("text"):
                cmd = str(child.text.strip())
        bpmn_graph.add_annotation(id, BPMN.TextAnnotation(id=id, name=name, command=cmd, process=process))
    elif tag.endswith("association") and not tag.endswith("dataoutputassociation") and not tag.endswith("datainputassociation"):
        id = curr_el.get("id")
        name = curr_el.get("name").replace("\r", "").replace("\n", "") if "name" in curr_el.attrib else ""
        source_ref = curr_el.get("sourceRef")
        target_ref = curr_el.get("targetRef")
        if source_ref in nodes_dict:
            bpmn_graph.find_annotation_association(nodes_dict[source_ref], target_ref)
    ...
    for child in curr_el:
        bpmn_graph = parse_element(bpmn_graph, counts, child, list(parents) + [child], incoming_dict, outgoing_dict, flows_name, 
                                   nodes_dict, nodes_bounds, flow_info, data_object_ref_dict, process=process, node=node,
    ...
    if rec_depth == 0:
        ...
        for flow_id in flow_info:
            if flow_id in outgoing_dict and flow_id in incoming_dict:
                if flows_name[flow_id] is not None:
                    flow = BPMN.SequenceFlow(outgoing_dict[flow_id][0], incoming_dict[flow_id][0], id=flow_id, name=flows_name[flow_id], process=outgoing_dict[flow_id][1])
                else:
                ...
                bpmn_graph.add_flow_details(flow)
                flow.del_waypoints()
...
def import_xml_tree_from_root(root):
    ...
    return parse_element(bpmn_graph, counts, root, [], incoming_dict, outgoing_dict, flows_name, nodes_dict, nodes_bounds,
                         flow_info, data_object_ref_dict)
